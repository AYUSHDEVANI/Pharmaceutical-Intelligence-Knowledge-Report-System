"""
PubChem MCP Server — Router
=============================
Defines the API endpoints:
  POST /query   — query PubChem for drug information
...
logger = logging.getLogger("mcp.pubchem.router")

router = APIRouter()

# The connector instance is attached at app startup (see main.py)
_connector: PubChemConnector | None = None


def set_connector(connector: PubChemConnector) -> None:
    """Called by main.py during startup to inject the connector."""
    global _connector
    _connector = connector


def get_connector() -> PubChemConnector:
...
@router.post(
    ...
    summary="Query PubChem for drug information",
    description=(
        "Accepts a drug name (and optional identifiers) and returns "
        "molecular formula, molecular weight, IUPAC name, SMILES, "
        "InChI, and InChIKey from PubChem."
    ),
)
async def query_drug(body: MCPRequest, request: Request) -> MCPResponse:
    """Query PubChem and return results in the standard MCP envelope."""
...
async def health_check() -> HealthResponse:
    """Return server health including PubChem upstream reachability."""
    connector = get_connector()
